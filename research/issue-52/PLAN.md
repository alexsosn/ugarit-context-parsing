...
4. shallow exact-commit fetch (`--depth 1`) followed by detached checkout;
...
Expected RED: the existing README already has installation/module/CUC identity and the no-auto-download statement, but lacks the actual CUC acquisition commands and still uses `/path/to/cuc/tf/0.2.8` in examples.
...
- show install → exact CUC shallow fetch/detached checkout → CSV module command;
- keep the detailed CSV/PDF subsections below it;
- replace or supplement placeholder CUC paths so the first runnable path is concrete;
...
Run the normal repository test matrix and existing consumer/Agora contracts on one frozen head. Since this is README-only after the RED test, no new external workflow is required.
